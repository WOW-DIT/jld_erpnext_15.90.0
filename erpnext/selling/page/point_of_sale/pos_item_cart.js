erpnext.PointOfSale.ItemCart = class {
	...
	bind_events() {
		...
		this.$component.on("change", "#use_deposit_checkbox", () => {
			const frm = this.events.get_frm();
			const deposit = this.customer_info?.deposit_balance || 0;
			const grand_total = frm.doc.rounded_total || frm.doc.grand_total;

			if ($("#use_deposit_checkbox").is(":checked")) {
				const amount_to_use = Math.min(deposit, grand_total);

				frm.set_value("use_deposit", 1);
				frm.set_value("deposit_used", amount_to_use);
				frm.set_value("paid_amount", grand_total - amount_to_use);
			} else {
				frm.set_value("use_deposit", 0);
				frm.set_value("deposit_used", 0);
	...
	render_grand_total(value) {
		...
		const frm = this.events.get_frm();
		const deposit_used = frm.doc.deposit_used || 0;
		const final_total = value - deposit_used;

		this.$totals_section
			.find(".grand-total-container")
			.html(`
				<div>${__("Grand Total")}</div>
				<div>${format_currency(final_total, currency)}</div>
				${deposit_used ? `<div style="font-size:12px;color:var(--gray-700)">
					(Deposit Used: -${format_currency(deposit_used, currency)})
				</div>` : ""}
			`);
		...
		if (show) {
			...
			this.$customer_section.find(".customer-details").html(
				...
				<div class="customer-fields-container">
					...
					<div class="deposit_balance-field"></div>
				</div>
